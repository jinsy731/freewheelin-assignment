 ...
 *
 * MockMvc를 사용하여 HTTP 요청/응답을 테스트합니다. (Spring Security 비활성화)
 */
...
class PieceControllerTest {
    ...
    fun `PATCH piece pieceId order - 문제 순서 수정 성공`() {
        ...
        val requestDto = ProblemOrderUpdateRequestDto(
            pieceProblemId = 503L,  // PieceProblem ID로 변경
            prevPieceProblemId = 501L,  // 이전 PieceProblem ID
            nextPieceProblemId = 502L   // 다음 PieceProblem ID
        )
    ...
    fun `PATCH piece pieceId order - 잘못된 요청 데이터로 실패`() {
        // Given - PieceProblem ID가 없는 경우
        val pieceId = 1L
        val invalidRequestDto = mapOf(
            "prevPieceProblemId" to 501L,
            "nextPieceProblemId" to 502L
            // pieceProblemId 누락
        )
